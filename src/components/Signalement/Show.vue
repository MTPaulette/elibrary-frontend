                      ...
                      >
                        ...
                        <td>
                          <router-link
                            :to="{
                              name: 'Document',
                              params: { document: signalement.Document },
                              query: { id: signalement.DocumentId },
                            }"
                          >
                            <span>{{ signalement.Document.titre }}</span>
                          </router-link>
                          <span
                            class="badge badge-success pull-right m-t-6"
                            v-if="signalement.Document.TypeId == 1"
                            >{{ signalement.Document.TypeId }}</span
                          >
                          <span
                            class="badge badge-info pull-right m-t-6"
                            v-else-if="signalement.Document.TypeId == 2"
                            >{{ signalement.Document.TypeId }}</span
                          >
                          <span
                            class="badge badge-danger pull-right m-t-6"
                            v-else-if="signalement.Document.TypeId == 3"
                            >{{ signalement.Document.TypeId }}</span
                          >
                          <span
                            class="badge badge-secondary pull-right m-t-6"
                            v-else
                            >{{ signalement.Document.TypeId }}</span
                          >
                        </td>
                        <td v-if="signalement.description">
...
export default {
  ...
  methods: {
    async getSignalements() {
      await axios
        .get("http://localhost:5000/api/signalements/signalements")
        .then((res) => {
